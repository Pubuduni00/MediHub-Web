  ...
  return (
    <div>
      <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',marginBottom:18,flexWrap:'wrap',gap:12}}>
        <p style={{fontSize:13.5,color:'var(--text-muted)'}}>{unreadCount} unread · {alerts.length} total</p>
        {unreadCount>0&&<button className="btn btn-ghost btn-sm" onClick={markAllAlertsRead}><CheckCheck size={14}/> Mark All Read</button>}
      </div>
