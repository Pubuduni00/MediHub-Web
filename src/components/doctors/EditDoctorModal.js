...
export default function EditDoctorModal({ isOpen, onClose, doctor }) {
  ...
  const validate = () => {
    ...
    if (!form.department || !form.department.trim()) e.department = 'Department is required';
    return e;
  };
      ...
      footer={
        success ? (
          ...
          <>
            ...
            <button className="btn btn-primary" onClick={handleSave}><Stethoscope size={14}/> Save Changes</button>
          </>
        )
      }
    >
      {success ? (
        <div style={{ textAlign: 'center', padding: '28px 0' }}>
          <div style={{ width: 60, height: 60, borderRadius: '50%', background: 'var(--accent-green-light)', display: 'flex', alignItems: 'center', justifyContent: 'center', margin: '0 auto 14px' }}>
            <Stethoscope size={28} color="var(--accent-green)" />
          </div>
          <h3 style={{ fontSize: 17, marginBottom: 6 }}>Doctor Details Updated Successfully!</h3>
          <p style={{ color: 'var(--text-muted)', fontSize: 13.5 }}>
            Changes for <strong>{doctor.name}</strong> have been saved in the database.
          </p>
        </div>
      ) : (
        <div>
          ...
          <div className="grid-2">
            ...
            <div className="form-group">
              ...
              <select className="form-control" value={form.specialty || 'General Medicine'} onChange={e => set('specialty', e.target.value)}>
