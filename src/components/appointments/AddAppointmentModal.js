...
export default function AddAppointmentModal({ isOpen, onClose, prefillDate='' }) {
  ...
  const [form, setForm] = useState({ ...EMPTY, date: prefillDate });
  ...
  const handleClose = () => { setSaved(false); setForm({...EMPTY,date:prefillDate}); setErrors({}); onClose(); };
      ...
      {saved ? (
        ...
        <div>
          ...
          <div className="form-group">
            ...
            <textarea className="form-control" rows={2} value={form.details} onChange={e=>set('details',e.target.value)} placeholder="Reason for visit, special instructions..." />
          </div>
        </div>
      )}
    </Modal>
